...
class MongoDBService:
    ...
    def connect(self):
        ...
        try:
            ...
            self.logger.info(
                f"Conectando a MongoDB: {mongo_host}:{mongo_port}/{mongo_database}"
            )
            self.logger.info(f"Usuario: {mongo_username}")
            self.logger.info(
                f"Autenticación requerida: {bool(mongo_username and mongo_password)}"
            )
            ...
            self.logger.info(
                f"URI de conexión: {mongo_uri.replace(mongo_password, '***') if mongo_password else mongo_uri}"
            )

            # Establecer conexión
            self.connection = connect(
                db=mongo_database, host=mongo_uri, alias="default"
            )
